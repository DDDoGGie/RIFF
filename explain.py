...
def build_args():
    ...
    parser.add_argument("--data_folder", type=str, default="/home/wcy/code/pyFile/RIFF/data/10X/DLPFC/")
...
def main(args):
    GSG.set_random_seed(args.seeds)
    ...
    adata, graph = GSG.build_graph(args, adata, need_preclust=False)

    adata_path = os.path.join(args.output_folder, "adata/"+args.sample_name+".h5ad")
    adata_imputed = sc.read_h5ad(adata_path)
    adata_imputed.X = adata_imputed.obsm["SEA_imputation_embedding"]
    adata_imputed.X[np.where(adata_imputed.X<0)] = 0
    
    selected_feats = set()
    for i in range(num_classes):
        selected_feat = GSG.find_influential_component(args, adata_imputed, graph, i)
    ...
    MoransI = GSG.compute_Moran_mean(adata, graph, svg_path)
