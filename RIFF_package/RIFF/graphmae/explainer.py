...
class Explainer:
    def __init__(
        ...
        self.pred = pred.to(self.device)
        self.adj = adj.to(self.device)
        self.neighbors = self.get_neighbors_id(cluster_id).to(self.device)
...
class ExplainModule(nn.Module):
    ...
    def loss(self, pred, pred_label, mi_obj=False):
        ...
        # contrast
        similarity_matrix = (self.x.detach() * feat_mask)@((self.x.detach() * feat_mask).T)
        positive = torch.sum(self.adj * similarity_matrix)
        negative = torch.sum(similarity_matrix)
        contrast_loss = self.coeffs["con"] * -torch.log(positive/negative)
